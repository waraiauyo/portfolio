            ...
            <div className="basis-1/3 flex gap-4 justify-center border-x-2">
                {
                    items.map((item) => {
                        return(
                            <Link key={item.name}e href={item.href} className={"px-4 py-2 transition rounded-[1rem] hover:bg-accent " + (pathname === item.href ? "bg-border hover:bg-border text-ice" : null)}>{item.name}</Link>
                        );
                    })
                }
            </div>
